...
### Правила анализа гипотез и первый разбор

- Написан **`hypotheses/ANALYSIS_RULES.md`** — правила анализа идей агентом: формат ответа, порядок работы, принципы (доказательность, честность, связь с проектом).
- Создан **`hypotheses/Opus4.6_Answer_Графы_pancake_и_остовы_асимптотика_2026_03_04_06_06_04.md`** — детальный разбор гипотезы о спектральных свойствах pancake-графов Кэли/Шрейера и асимптотике числа остовных деревьев. Вердикт: ЧАСТИЧНО ВЕРНА (основной результат для регулярного Cayley-графа корректен, но тривиален; Schreier-часть требует доработки).

---

## 2026-02-27

### Папки для статьи и гипотез

- Добавлена папка **`article/`** — для написания статьи по теме проекта (LaTeX). Внутри: `WRITING_RULES.md` — документация по правилам написания статьи.
- Добавлена папка **`hypotheses/`** — для гипотез и идей, которые агент проверяет по данным проекта. Внутри: `ANALYSIS_RULES.md` — документация по правилам анализа.
- Обновлены `docs/00_AGENT_NAVIGATION.md` (структура репозитория и таблица «Где что искать») и `README.md` (раздел «Документация» и «Структура»).
...
- В проекте: скрипт `run_best_score.py` переведён на режимы `--mode notebook` (только v4, цель 89980) и `--mode beam` (только baseline+beam, цель 91584); в актуальной структуре вызывается через обёртку `run_best.py`. Режимы crossing оставлены как экспериментальные.
...
- Добавлен **`run_research.py`**: единый скрипт для пакетного прогона всех доступных методов с поддержкой resume через `state.json` (сейчас реализован в `scripts/runners/run_research.py`, удобный вход — через `run_experiment.py research`).
  ...
  - Запуск (исторически): `python run_research.py` или `python run_research.py --profile full --limit 300`; в новой структуре — через `python run_experiment.py research --profile full --limit 300`.
